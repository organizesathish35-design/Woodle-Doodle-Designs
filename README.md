...
- **Softer motion on touch.** The reveal spring is tuned per device: on a phone the same animation has to resolve inside a much smaller frame, and overshoot reads as jitter rather than bounce. Touch gets shorter travel (22px instead of 40px), almost no bounce, a slightly longer settle, and fires ~8% of a viewport before the element arrives so it is already settling by the time you look at it. Desktop keeps the livelier original.
- **Steady nav on touch.** The bar hid and reappeared on a per-frame `dy > 3` test, which flips constantly while a finger wobbles. A hesitant drag toggled it **119 times**; it now accumulates 48px of travel in one direction before acting, measured at **0** toggles for the same gesture. Desktop still uses the original per-frame test (verified: 119 toggles at 1440px, unchanged).
